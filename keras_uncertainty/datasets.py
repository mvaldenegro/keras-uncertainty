...
def toy_regression_monotonic_sinusoid(num_samples=500, ood_samples=0):
    """
    ...
    def fn(x):
        ...
        return x * math.sin(x) + e1 * x + e2

    x_train, y_train = sample_function(fn, [0.0, 10.0], size=num_samples)

    if ood_samples > 0:
        x_test, y_test = sample_function(fn, [10.0, 15.0], size=ood_samples)
